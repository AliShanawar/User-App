...
class PriceConverter {
  static String convertPrice(BuildContext context, double price,
    ...
    bool _inRight = Provider.of<SplashProvider>(context, listen: false)
        ...
        'right';
    return '${_inRight ? '' : Provider.of<SplashProvider>(context, listen: false).myCurrency.symbol}${(price * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)).toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
